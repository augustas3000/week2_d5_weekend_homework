...
class Bar
  ...
  def initialize()
    # initialize with a default @bar_stock variable that points to hash of alko names strings as keys and
    # hashes containing default stock(in serving units) and unit price(float). As drinks get sold stock will be updated:
    @bar_stock = {
  ...
  # add drink to tab function explained: takes in guest object that was inputed to sell_a_drink function as well as
  # his/her chosen drink as a string.
  def add_drink_to_tab(guest_obj, drink_name_str)
    # create a hash value for guest_obj key which is to be
    # added to the @tab hash. this will be utilised to update @tab hash in case,
    # specific guest obj does not yet exist in @tab hash:
    ...
    if @tab.has_key?(guest_obj)
      ...
      if @tab[guest_obj][:drinks_bought].has_key?(drink_name_str)
        ...
        # if drink_str does not exist yet as a key we use = assingment operator to 1 int, thsi way next time += will work as ruby no longer assumes a nil value, for key that does not yet exist.
        @tab[guest_obj][:drinks_bought][drink_name_str] = 1
      end

    else
      # in case guest_obj in question is not yet among keys in @tab array, we add it and point it guest_obj_hash_1_buy variable taht links to the hash template we created at teh start of function.
